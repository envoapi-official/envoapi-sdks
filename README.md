...
Local SDK repository for TypeScript/JavaScript, Python, and Go. All three clients cover the 52 operations in the committed public OpenAPI 3.1 snapshot. Builds and installed clients do not depend on the backend repository or its private contracts package.

The packages are versioned `0.1.0` for local development. They have not been published. The intended repository is `envoapi/sdks`; the npm and PyPI package names are `envoapi`, and the Go module is `github.com/envoapi/sdks/go`.
...
GitHub setup, Actions, registry ownership, public licensing, and publishing are deferred. Before public release, select the repository license and verify package names and permissions. Future Go tags must include the module subdirectory, for example `go/v0.1.0`; each language can release independently. Future Actions jobs must use runner group `envoapi-runner` and labels `[self-hosted, linux, x64]`.
